...
RAISE NOTICE 'Phase 1: Adding company_id columns...';
...
RAISE NOTICE 'Phase 1 complete: All columns added';

-- =============================================================================
-- PHASE 2: Populate company_id from parent tables
-- =============================================================================

RAISE NOTICE 'Phase 2: Populating company_id values...';
...
RAISE NOTICE 'Phase 2 complete: All data populated';
...
BEGIN
    ...
    IF total_orphaned > 0 THEN
        RAISE WARNING 'Found orphaned rows:';
        IF orphaned_pws > 0 THEN RAISE WARNING '  - product_warehouse_stock: % rows', orphaned_pws; END IF;
        IF orphaned_ii > 0 THEN RAISE WARNING '  - invoice_items: % rows', orphaned_ii; END IF;
        IF orphaned_jl > 0 THEN RAISE WARNING '  - journal_lines: % rows', orphaned_jl; END IF;
        IF orphaned_sti > 0 THEN RAISE WARNING '  - stock_transfer_items: % rows', orphaned_sti; END IF;
        IF orphaned_ec > 0 THEN RAISE WARNING '  - employee_contracts: % rows', orphaned_ec; END IF;
        IF orphaned_elr > 0 THEN RAISE WARNING '  - employee_leave_requests: % rows', orphaned_elr; END IF;
        IF orphaned_erd > 0 THEN RAISE WARNING '  - employee_recurring_deductions: % rows', orphaned_erd; END IF;

        RAISE EXCEPTION 'Migration aborted: Found % orphaned rows without company_id. Please investigate and fix manually.', total_orphaned;
    END IF;

    RAISE NOTICE 'Verification passed: No orphaned rows found';
END $$;
...
RAISE NOTICE 'Phase 4: Adding NOT NULL constraints...';
...
RAISE NOTICE 'Phase 4 complete: All NOT NULL constraints added';

-- =============================================================================
-- PHASE 5: Add foreign key constraints
-- =============================================================================

RAISE NOTICE 'Phase 5: Adding foreign key constraints...';
...
RAISE NOTICE 'Phase 5 complete: All foreign keys added';

-- =============================================================================
-- PHASE 6: Create indexes for performance
-- =============================================================================

RAISE NOTICE 'Phase 6: Creating indexes...';
...
RAISE NOTICE 'Phase 6 complete: All indexes created';
...
BEGIN
    ...
    RAISE NOTICE '  - product_warehouse_stock: % rows', pws_count;
    RAISE NOTICE '  - invoice_items: % rows', ii_count;
    RAISE NOTICE '  - journal_lines: % rows', jl_count;
    RAISE NOTICE '  - stock_transfer_items: % rows', sti_count;
    RAISE NOTICE '  - employee_contracts: % rows', ec_count;
    RAISE NOTICE '  - employee_leave_requests: % rows', elr_count;
    RAISE NOTICE '  - employee_recurring_deductions: % rows', erd_count;
END $$;

RAISE NOTICE 'Phase 7 complete: Verification successful';
...
RAISE NOTICE '1. Restart backend application to pick up schema changes';
RAISE NOTICE '2. Monitor logs for any errors related to company_id';
RAISE NOTICE '3. Run post-migration verification queries';
RAISE NOTICE '4. Update application code to use company_id in child table queries';
RAISE NOTICE '=============================================================================';
...
-- =============================================================================
-- ROLLBACK;
-- RAISE NOTICE 'Migration rolled back. Check errors above.';
