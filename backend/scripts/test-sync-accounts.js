...
const run = async () => {
  ...
  try {
    ...
    for (const item of initialAccounts) {
      ...
      console.log(`Syncing item ${item.id} -> code ${item.code} -> targetId ${targetId}`);
      try {
        await client.query(
          ...
          [targetId, companyId, item.code, item.name, item.type, !!item.isGroup, item.currency || 'ILS', Number(item.balance || 0), item.isActive !== false]
        );
      } catch (err) {
