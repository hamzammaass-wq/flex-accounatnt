    ...
    for (const op of operations) {
      ...
      if (type === 'delete') {
        ...
        if (collectionName === 'transactions') {
          ...
          await client.query(`DELETE FROM journal_lines WHERE entry_id = $1`, [data.id]);
          for (const line of lines) {
            await client.query(
              `INSERT INTO journal_lines (entry_id, account_id, debit, credit, currency, exchange_rate, note)
               VALUES ($1, $2, $3, $4, $5, $6, $7)`,
              [data.id, prefixAccountId(companyId, line.accountId), Number(line.debit || 0), Number(line.credit || 0), line.currency || data.currency, Number(line.exchangeRate || data.exchangeRate || 1.0), line.note || null]
            );
          }
        } else if (collectionName === 'invoices') {
          await client.query(
          ...
          if (Array.isArray(data.items)) {
            for (const details of data.items) {
              await client.query(
                ...
                [details.id, data.id, details.productId || null, prefixAccountId(companyId, details.accountId), details.description || '', Number(details.quantity || 0), Number(details.unitPrice || 0), Number(details.total || 0), !!details.returned, details.width ? Number(details.width) : null, details.length ? Number(details.length) : null]
              );
            }
          }
        }
