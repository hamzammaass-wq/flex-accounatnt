      ...
      if (collectionName === 'accounts') {
        ...
        for (const item of upserts) {
          if (collectionName === 'contacts') {
            ...
            await client.query(
              `INSERT INTO employee_contracts (id, company_id, employee_id, contract_type, start_date, end_date, status, notes)
              ...
              [item.id, companyId, item.employeeId, item.contractType || 'OPEN_ENDED', parseDate(item.startDate), parseDate(item.endDate), item.status || 'ACTIVE', item.notes || null]
            );
          }

          else if (collectionName === 'employeeLeaveRequests') {
            await client.query(
              `INSERT INTO employee_leave_requests (id, company_id, employee_id, leave_type, status, effective_from, effective_to, days, note, deduct_from_payroll)
              ...
              [item.id, companyId, item.employeeId, item.leaveType, item.status || 'PENDING', parseDate(item.effectiveFrom), parseDate(item.effectiveTo), Number(item.days || 0), item.note || null, !!item.deductFromPayroll]
            );
          }

          else if (collectionName === 'employeeRecurringDeductions') {
            await client.query(
              `INSERT INTO employee_recurring_deductions (id, company_id, employee_id, type, status, label, amount, effective_from, effective_to)
              ...
              [item.id, companyId, item.employeeId, item.type, item.status || 'ACTIVE', item.label, Number(item.amount || 0), parseDate(item.effectiveFrom), parseDate(item.effectiveTo)]
            );
          }
          ...
          if (Array.isArray(item.items)) {
            for (const details of item.items) {
              await client.query(
                ...
                [details.id, companyId, item.id, details.productId || null, prefixAccountId(companyId, details.accountId), details.description || '', Number(details.quantity || 0), Number(details.unitPrice || 0), Number(details.total || 0), !!details.returned, details.width ? Number(details.width) : null, details.length ? Number(details.length) : null]
              );
            }
          }
        }
            ...
            for (const ws of item.warehouseStock) {
              await client.query(
                `INSERT INTO product_warehouse_stock (company_id, product_id, warehouse_id, quantity)
                 VALUES ($1, $2, $3, $4)`,
                [companyId, item.id, ws.warehouseId, Number(ws.quantity || 0)]
              );
            }
          }
        }
          ...
          if (Array.isArray(item.items)) {
            for (const detail of item.items) {
              await client.query(
                `INSERT INTO stock_transfer_items (company_id, transfer_id, product_id, quantity, description)
                 VALUES ($1, $2, $3, $4, $5)`,
                [companyId, item.id, detail.productId, Number(detail.quantity || 0), detail.description || null]
              );
            }
          }
        }
