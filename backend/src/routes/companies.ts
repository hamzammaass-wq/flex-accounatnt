...
router.get('/', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        if (Array.isArray(firestoreCompanies) && firestoreCompanies.length > 0) {
          ...
          const deletedIds = new Set(deletedRes.rows.map((r: any) => r.id));

          for (const fc of firestoreCompanies) {
            if (!fc.id || fc.id === 'cmp_default') continue;
    ...
    let result = await query(
      `SELECT c.*, m.role as user_role, m.status as user_status
       FROM companies c
...
router.put('/:companyId', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
...
// Delete company
router.delete('/:companyId', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  const { companyId } = req.params;
  const uid = req.user?.uid;
  ...
  try {
    ...
    await query(
      `INSERT INTO deleted_companies (id) VALUES ($1) ON CONFLICT (id) DO NOTHING`,
      [companyId]
    );
    ...
    try {
      ...
      if (userDoc.exists) {
        const userData = userDoc.data();
        const firestoreCompanies: any[] = userData?.companies || [];
        const updatedCompanies = firestoreCompanies.filter((fc: any) => fc.id !== companyId);
...
router.post('/:companyId/clear', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  const { companyId } = req.params;
  const userMembership = (req as any).membership;
  ...
  try {
    ...
    await client.query(
      'DELETE FROM journal_lines WHERE entry_id IN (SELECT id FROM journal_entries WHERE company_id = $1)',
      [companyId]
    );
    // 7. Journal entries
    ...
    await client.query(
      'DELETE FROM stock_transfer_items WHERE transfer_id IN (SELECT id FROM stock_transfers WHERE company_id = $1)',
      [companyId]
    );
    // 9. Stock transfers
    ...
    await client.query(
      'DELETE FROM product_warehouse_stock WHERE product_id IN (SELECT id FROM products WHERE company_id = $1)',
      [companyId]
    );
    // 11. Products
    ...
    await client.query(
      'DELETE FROM employee_recurring_deductions WHERE employee_id IN (SELECT id FROM employees WHERE company_id = $1)',
      [companyId]
    );
    await client.query(
      'DELETE FROM employee_leave_requests WHERE employee_id IN (SELECT id FROM employees WHERE company_id = $1)',
      [companyId]
    );
    await client.query(
      'DELETE FROM employee_contracts WHERE employee_id IN (SELECT id FROM employees WHERE company_id = $1)',
      [companyId]
    );
    // 14. Employees
