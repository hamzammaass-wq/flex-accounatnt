...
router.post('/:companyId/clear', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    await client.query(
      'DELETE FROM invoice_items WHERE company_id = $1',
      [companyId]
    );
    // 5. Invoices
    ...
    await client.query(
      'DELETE FROM stock_transfer_items WHERE company_id = $1',
      [companyId]
    );
    // 9. Stock transfers
    ...
    await client.query(
      'DELETE FROM product_warehouse_stock WHERE company_id = $1',
      [companyId]
    );
    // 11. Products
    ...
    await client.query(
      'DELETE FROM employee_recurring_deductions WHERE company_id = $1',
      [companyId]
    );
    await client.query(
      'DELETE FROM employee_leave_requests WHERE company_id = $1',
      [companyId]
    );
    await client.query(
      'DELETE FROM employee_contracts WHERE company_id = $1',
      [companyId]
    );
    // 14. Employees
