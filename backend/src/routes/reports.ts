...
router.get('/ledger-card/:accountId', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    // Get opening balance before the start date from POSTED entries only.
    ...
    // Get period details from POSTED entries only.
...
router.get('/contact-statement/:contactId', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    // Debit/credit must come from the contact's actual control-account journal lines.
    // Deriving direction from the journal header type is incorrect for receipts/payments.
    const detailsResult = await query(
...
router.get('/dashboard-summary', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  const { companyId } = req.params;
  try {
    const result = await query(
      `SELECT
