...
router.post('/', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  ...
  const {
    ...
    exchangeRate,
    status,
    lines // Array of { accountId, debit, credit, note }
  } = req.body;
  ...
  try {
    ...
    for (const line of lines) {
      ...
      await client.query(
        ...
        [balanceChange, prefixedAccountId, companyId]
      );
    }
...
router.post('/:transactionId/reverse', verifyCompanyMembership, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const originalHeader = await client.query(
    ...
    for (const originalLine of originalLines.rows) {
      ...
      await client.query(
        `UPDATE accounts
         SET balance = balance + $1
         WHERE id = $2 AND company_id = $3`,
        [balanceChange, originalLine.account_id, companyId]
      );
    }
