...
router.post('/clean-tenant-overlap', async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid;
  ...
  try {
    ...
    res.json({
      ...
      firestoreUpdates: firestoreUpdates.map(u => ({ userId: u.userId, oldList: u.oldList, newList: u.newList.map((c: any) => c.id) }))
    });
