 ...
 */
export const prefixAccountId = (companyId: string, id: string | null | undefined): string | null => {
  if (!id) return null;
 ...
 */
export const unprefixAccountId = (companyId: string, id: string | null | undefined): string | null => {
  if (!id) return null;
 ...
 */
export const resolveDbAccountId = async (
  client: any,
  companyId: string,
  ...
  try {
    ...
    await client.query(
      `INSERT INTO accounts (id, company_id, code, name, type, currency, is_active)
