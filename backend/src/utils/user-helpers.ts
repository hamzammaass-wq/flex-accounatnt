...
export async function upsertUser(
  ...
  if (subscription) {
    await query(
      `INSERT INTO users (id, email, name, role, picture, account_code, subscription)
       ...
       SET email = EXCLUDED.email, 
           ...
           subscription = COALESCE(users.subscription, EXCLUDED.subscription)`,
      [uid, email || '', name || '', role, picture || null, accountCode, typeof subscription === 'string' ? subscription : JSON.stringify(subscription)]
    );
  } else {
    await query(
      `INSERT INTO users (id, email, name, role, picture, account_code)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (id) DO UPDATE
       SET email = EXCLUDED.email, 
           name = COALESCE(users.name, EXCLUDED.name), 
           ...
           picture = COALESCE(users.picture, EXCLUDED.picture),
           account_code = COALESCE(users.account_code, EXCLUDED.account_code)`,
      [uid, email || '', name || '', role, picture || null, accountCode]
    );
  }
