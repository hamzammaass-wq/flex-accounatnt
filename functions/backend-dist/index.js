...
app.post('/api/companies/:companyId/users/:userId/change-password', authenticateUser, async (req, res) => {
...
app.post('/api/admin/sync-codes', authenticateUser, async (req, res) => {
    if (req.user?.email !== 'hamza.mm.aa.ss@gmail.com') {
...
app.post('/api/admin/users/:userId/edit', authenticateUser, async (req, res) => {
    const { userId } = req.params;
...
app.post('/api/companies/:companyId/users/create', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        // Create membership
        await query(`INSERT INTO memberships (company_id, user_id, role, status) VALUES ($1, $2, $3, 'ACTIVE')
       ON CONFLICT (company_id, user_id) DO NOTHING`, [companyId, userRecord.uid, role]);
        ...
        for (const op of operations) {
            ...
            if (type === 'delete') {
             ...
             SET voucher_id = EXCLUDED.voucher_id, amount = EXCLUDED.amount, description = EXCLUDED.description,
               ...
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [companyId, data.id, await resolveDbAccountId(client, companyId, line.accountId), Number(line.debit || 0), Number(line.credit || 0), line.currency || data.currency, Number(line.exchangeRate || data.exchangeRate || 1.0), line.note || null]);
                    }
                }
                else if (collectionName === 'invoices') {
