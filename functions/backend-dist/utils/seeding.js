...
export async function seedNewCompany(client, companyId, baseCurrency = 'ILS') {
    // 1. Seed currencies
    ...
    for (const c of defaultCurrencies) {
       ...
       SET name = EXCLUDED.name, symbol = EXCLUDED.symbol, rate = EXCLUDED.rate`, [companyId, c.code, c.name, c.symbol, c.rate]);
    }
    ...
    for (const w of initialWarehouses) {
        await client.query(`INSERT INTO warehouses (id, company_id, name, location, is_main)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (company_id, id) DO UPDATE
       SET name = EXCLUDED.name, location = EXCLUDED.location, is_main = EXCLUDED.is_main`, [w.id, companyId, w.name, w.location, w.is_main]);
    }
    ...
    for (const item of initialAccounts) {
       ...
       SET code = EXCLUDED.code, name = EXCLUDED.name, type = EXCLUDED.type, parent_id = NULL,
           is_group = EXCLUDED.is_group, currency = EXCLUDED.currency, balance = EXCLUDED.balance, is_active = EXCLUDED.is_active`, [prefixedId, companyId, item.code, item.name, item.type, !!item.isGroup, item.currency, Number(item.balance || 0), true]);
    }
    ...
    for (const item of initialAccounts) {
        if (item.parentId) {
           ...
           SET parent_id = $1
           WHERE company_id = $2 AND id = $3`, [prefixedParentId, companyId, prefixedChildId]);
            }
        }
    }
}
