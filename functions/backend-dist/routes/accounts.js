...
router.post('/', verifyCompanyMembership, async (req, res) => {
    ...
    try {
        const result = await query(`INSERT INTO accounts (id, company_id, code, name, type, parent_id, is_group, currency, balance)
...
// Delete an account
router.delete('/:accountId', verifyCompanyMembership, async (req, res) => {
    const { companyId, accountId } = req.params;
    try {
        ...
        const result = await query(`DELETE FROM accounts WHERE company_id = $1 AND id = $2 RETURNING *`, [companyId, prefixedAccountId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Account not found' });
        }
        const row = result.rows[0];
