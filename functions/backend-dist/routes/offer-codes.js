...
router.get('/', async (req, res) => {
    ...
    try {
        const result = await query('SELECT * FROM workspace_offer_codes ORDER BY created_at DESC LIMIT 200');
...
router.post('/', async (req, res) => {
    ...
    try {
        await query(`INSERT INTO workspace_offer_codes
...
router.get('/:codeId', async (req, res) => {
    ...
    try {
        const result = await query('SELECT * FROM workspace_offer_codes WHERE code = $1', [codeId.trim().toUpperCase()]);
...
router.post('/:codeId/redeem', async (req, res) => {
    ...
    try {
        ...
        const result = await query(`UPDATE workspace_offer_codes
       SET status = 'USED',
           used_at = NOW(),
           used_by_user_id = $2,
           used_by_email = $3
