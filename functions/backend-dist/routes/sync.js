...
router.get('/:collectionName', verifyCompanyMembership, async (req, res) => {
    ...
    try {
        ...
        if (collectionName === 'transactions') {
         ...
         WHERE m.company_id = $1`, [companyId]);
        }
        else if (collectionName === 'accounts') {
            result = await query(`SELECT a.*,
           COALESCE((
             SELECT SUM(
               CASE 
        ...
        if (collectionName === 'transactions' && result) {
            const mappedRows = result.rows.map((row) => {
                const debitLine = (row.lines || []).find((l) => Number(l.debit || 0) > 0);
                const creditLine = (row.lines || []).find((l) => Number(l.credit || 0) > 0);
                return {
                    id: row.id,
                    date: row.date,
...
router.post('/:collectionName/sync', verifyCompanyMembership, async (req, res) => {
    const { companyId, collectionName } = req.params;
    ...
    if (!dbTable) {
        ...
        try {
            ...
            if (Array.isArray(deletes) && deletes.length > 0) {
                ...
                collectionData = collectionData.filter((item) => !deleteSet.has(item.id));
            }
            // Process upserts
            if (Array.isArray(upserts) && upserts.length > 0) {
                const upsertMap = new Map(upserts.map((item) => [item.id, item]));
         ...
         SET settings = $1
         WHERE id = $2`, [JSON.stringify(settings), companyId]);
        ...
        if (Array.isArray(deletes) && deletes.length > 0) {
            if (collectionName === 'currencies') {
                ...
                if (collectionName === 'accounts') {
                    ...
                    targets = Array.from(new Set([...prefixed, ...unprefixed])).filter(Boolean);
                }
                await client.query(`DELETE FROM ${dbTable} WHERE company_id = $1 AND id = ANY($2)`, [companyId, targets]);
            }
        }
        // 2. Process upserts
        if (Array.isArray(upserts) && upserts.length > 0) {
            if (collectionName === 'accounts') {
             ...
             SET code = EXCLUDED.code, name = EXCLUDED.name, type = EXCLUDED.type, parent_id = NULL,
                ...
                for (const item of upserts) {
                    if (item.parentId) {
                        ...
                        const parentCheck = await client.query(`SELECT id FROM accounts WHERE company_id = $1 AND (id = $2 OR id = $3)`, [companyId, prefixedParentId, item.parentId]);
                        if (parentCheck.rows.length > 0) {
                            const dbParentId = parentCheck.rows[0].id;
                            const childCheck = await client.query(`SELECT id FROM accounts WHERE company_id = $1 AND (id = $2 OR id = $3)`, [companyId, prefixAccountId(companyId, item.id), item.id]);
                            if (childCheck.rows.length > 0) {
                                const dbChildId = childCheck.rows[0].id;
                                await client.query(`UPDATE accounts
                   SET parent_id = $1
                   WHERE company_id = $2 AND id = $3`, [dbParentId, companyId, dbChildId]);
                            }
                        }
                        else {
               ...
               VALUES ($1, $2, $3, 'ACTIVE')
               ON CONFLICT (company_id, user_id) DO NOTHING`, [companyId, item.id, item.role === 'OWNER' ? 'OWNER' : 'MEMBER']);
                    }
                    else if (collectionName === 'currencies') {
                        await client.query(`INSERT INTO currencies (company_id, code, name, symbol, rate)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (company_id, code) DO UPDATE
             SET name = EXCLUDED.name, symbol = EXCLUDED.symbol, rate = EXCLUDED.rate`, [companyId, item.code, item.name, item.symbol || null, Number(item.rate || 1.0)]);
                    }
             ...
             SET transfer_number = EXCLUDED.transfer_number, date = EXCLUDED.date,
                 ...
                 VALUES ($1, $2, $3, $4, $5)`, [companyId, item.id, detail.productId, Number(detail.quantity || 0), detail.description || null]);
                        ...
                        await client.query(`INSERT INTO fixed_asset_groups (
