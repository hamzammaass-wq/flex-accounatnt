 ...
 */
export const query = async (text, params, retries = 3, delay = 1000) => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            const isTransient = attempt < retries &&
            ...
            if (isTransient) {
                console.warn(`[Database Query Warning] Transient error on attempt ${attempt}/${retries}. Retrying in ${delay * attempt}ms... Error: ${error.message}`);
 ...
 */
export const getClient = async (retries = 3, delay = 1000) => {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            const isTransient = attempt < retries &&
                (error.code === '57P01' ||
                    ...
                    error.message?.includes('terminating'));
            if (isTransient) {
                console.warn(`[Database Connection Warning] Failed to connect on attempt ${attempt}/${retries}. Retrying in ${delay * attempt}ms... Error: ${error.message}`);
                await wait(delay * attempt);
            }
            else {
