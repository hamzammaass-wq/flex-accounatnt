...
describe('brand assets normalization', () => {
  it('maps legacy brand paths to the current bundled assets', () => {
    expect(normalizeBrandLogoUrl('/brand/flex-erp-logo.png', DEFAULT_BRAND_LOGO_URL)).toBe(DEFAULT_BRAND_LOGO_URL);
    expect(normalizeBrandLogoUrl('/brand/flex-erp-logo.svg', DEFAULT_BRAND_LOGO_URL)).toBe(DEFAULT_BRAND_LOGO_URL);
    expect(normalizeBrandLogoUrl('/brand/flex-erp-mark.svg')).toBe(DEFAULT_BRAND_MARK_URL);
  });
