...
const SalesInvoiceList: React.FC<SalesInvoiceListProps> = ({ onNavigate, onEditInvoice, onCreateReturn }) => {
    const { invoices, contacts, products, baseCurrency, companySettings, postInvoice, updateInvoice, deleteInvoice, returnInvoiceItem, reverseInvoice, currentCompanyId, loadMoreInvoices, setInvoiceDateRange, useBackend, invoicesLoading, invoicesHasMore } = useAccounting();
        ...
        <body>
          <div class="sheet">
              ...
              <div class="meta-cards">
                  <div class="meta-card">
                      <strong>${tr('إلى', 'Bill To')}</strong>
                      <span>${customerName}</span>
                      ${printPersonalData && customer?.phone ? `<div style="margin-top:4px;font-size:12px;color:var(--text-muted)">${tr('الجوال', 'Phone')}: <span dir="ltr">${customer.phone}</span></div>` : ''}
                  </div>
                  <div class="meta-card">
              ...
              <div class="signatures">
                  ...
                  <div class="signature-box">
                      <div class="signature-line"></div>
                      <div class="signature-label">${tr('توقيع العميل', 'Customer Signature')}</div>
                  </div>
              </div>
          </div>
    ...
    return (
        <div className="app-page sales-list-page animate-in fade-in duration-500 p-4" dir={isEnglish ? 'ltr' : 'rtl'}>
            ...
            {useBackend && invoicesHasMore && filteredList.length > 0 && (
