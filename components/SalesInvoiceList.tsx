...
const SalesInvoiceList: React.FC<SalesInvoiceListProps> = ({ onNavigate, onEditInvoice, onCreateReturn }) => {
    ...
    const getStatusConfig = (status: string) => {
        switch (status) {
            ...
            case 'QUOTATION': return { label: tr('مسودة عرض', 'Draft Quotation'), color: 'bg-amber-50 text-amber-600 border-amber-100', icon: <FilePlus size={12} /> };
                    ...
                    <p className="text-gray-400 text-xs font-bold mt-1 uppercase tracking-widest">
                        {activeTab === 'INVOICES'
                            ...
                            : activeTab === 'RETURNS'
                                ...
                                : tr('إدارة عروض الأسعار المسودة', 'Manage draft quotations')}
                    </p>
                </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {isDraft && activeTab !== 'QUOTATIONS' && <span className="text-[9px] bg-amber-50 text-amber-600 px-2 py-0.5 rounded font-black border border-amber-100">{tr('مسودة', 'Draft')}</span>}
                                            </div>
                                            <div className="flex items-center gap-1 text-[10px] font-black text-gray-400 uppercase tracking-wider"><User size={12} className="text-blue-400" />{getCustomerName(inv.customerId)}</div>
                                        </div>
                                    </div>
